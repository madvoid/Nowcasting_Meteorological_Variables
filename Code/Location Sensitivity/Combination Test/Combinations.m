...
for iter = 1:length(combi)
    ...
    for j = 1:numTargets
        ...
        [net, tr] = HeterogeneityANN(inputsTrain, targetsTrain(j,:), 'default', numHiddenNodes);   % Third input is random seed
        outputsTest(j,:) = net(inputsTest);
        netIters(iter, j) = tr.num_epochs;
        netFail{iter, j} = tr.stop;
    end
...
%% Save
save('CombinationStatisticsDefault2.mat', 'combi', 'mmRmse', 'stdRmse', 'rSquared', 'netIters', 'netFail', 'failedRuns');
